...
// Performs a warp-level reduction to compute the sum of values within a warp (CUDA version check included)
...
// Optimized GPU function for computing Euclidean distance with shared memory and warp-level reductions
template <typename T>
