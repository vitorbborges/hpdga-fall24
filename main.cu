...
int main() {
    ...
    std::vector<float> h_distances_simple(num_vectors);
    std::vector<float> h_distances_kepler(num_vectors);
    std::vector<float> h_distances_cpu(num_vectors); // For CPU results
    std::vector<float> h_distances_atomic(num_vectors);
    ...
    // Copy data to device
    cudaMemcpy(d_vec1, h_vec1.data(), num_vectors * dimensions * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_vec2, h_vec2.data(), num_vectors * dimensions * sizeof(float), cudaMemcpyHostToDevice);
    ...
    std::cout << "Simple GPU kernel time: "
              << std::chrono::duration_cast<std::chrono::nanoseconds>(end_simple - start_simple).count()
              << " ns" << std::endl;
    std::cout << "Optimized GPU kernel time: "
              << std::chrono::duration_cast<std::chrono::nanoseconds>(end_kepler - start_kepler).count()
              << " ns" << std::endl;
